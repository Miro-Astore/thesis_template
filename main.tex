...
\usepackage{epigraph}
\usepackage[backref=true,backend=bibtex,style=chem-acs,natbib=true,doi=true,isbn=false,arxiv=false]{biblatex}
\AtEveryCitekey{%
	\clearfield{note}%
}
...
%\newcommand{\chapquote}[3]{\noindent\begin{flushleft}\begin{quotation}\textit{#1}\end{quotation} \end{flushleft}\begin{flushright}#2\end{flushright}}
\makeatletter
...
%%%use this to quickly speed up your compilations, only the content of chapter 1 will chapter will be compiled if you use uncomment this line. 
...
\include{02-Authorship}
\include{03-Abstract}
\include{04-Acknowledgements}
\include{05-Publications}
...
\include{./07-Foreword}
\include{./08-Introduction}
\include{./09-Lit_review}
\include{./10-Results_1}
\include{./11-Results_2}
\include{./12-Results_3}
\include{./13-Conclusion}
\include{./15-Afterword}
\include{./16-Appendix}
