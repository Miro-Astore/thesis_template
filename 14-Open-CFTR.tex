...
A recent, careful, electrophysiology study into the kinetics of the R117H mutation has given important implications for the work presented here \cite{}. Before this study it was thought that R117 forms a salt bridge with E1126 \cite{cui2014}. But in fact, the experiments in the former publication found that it is E1124 which makes a connection to R117, by interacting with the carboxyl oxygen atom in the backbone of R117, not E1126. This situation leaves the role of E1126 in channel gating kinetics unknown, but through our \textit{in silico} experiments, we believe that E1126 forms a salt bridge with R334. THis may be tested through single ion channel electrophysiology experiments.

The work presented in this chapter differs in an important way from existing computational studies. Previous attempts to induce conformational changes have usually been \textit{targetted}, that is, they usually sought to resolve the energy landscape \textit{between} experimentally determined structural snapshots of the protein. Since we do not know what the conducting conformation of CFTR looks like, this study was \textit{untargetted}. And so we have combined hints from \textit{in vitro} biophysical experiments on CFTR with results from our unbiased MD simulations to choose collective variables which might let us resolve this conducting conformation. 

\section{Results}


\subsection{Long Simulations Reveal Motions which Dilate the Pore of CFTR}
4 simulations were run of WT-CFTR of at least 2 microseconds each, for a total of 8.2 $\mu s$ of unbiased MD sampling. In post processing, the motions of the transmembrane helices were analysed using Principal Component Anlaysis. This revelaed large motions in TM1 and TM6 which appeared as though they dilated the pore (Figure \ref{PCA_motions}). 
...
The first two principal components were chosen for further analysis as these two together represented 38\% of the kinetic variance in the unbiased data. Additionally, they produced motions in TM1 and TM6, which is where we expected motions to occur due to the placement of the experimentally determined selectivity filter \cite{linsdell2018, negoda2018, negoda2019}. 

\subsection{OPES-Metadynamics Finds a Stable Configutaion with an Open Pore }
The principal components derived in the previous section were used as collective variables to study the motion of the pore lining transmembrane helices. The energetics of these two motions was investigated through the use of 8 indepdent walkers, each running for $1.5 \mu s$ of OPES metadynamics. This method was used to constructed the free energy surface in figure \ref{summary_FES}A, and convergence is demonstrated in \ref{convergence_FES}.

This lead to the discovery of the free energy well at (0.10, 0.21) in Figure \ref{summary_FES}. By steering the system to this location in CV sapce and allowing it to equilibratie we noticed that the pore had been significnatly dilated, in the location where we expected the extracellular opening to form (). , it was found that the protein remained in an a conformation which dilated the pore to almost 3 $\AA$. This larger opening was in the right place between TM1 and TM6 and of a size which we would expect for anions known to pass through CFTR. 
