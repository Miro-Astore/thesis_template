...
\end{figure}
CFTR will not allow cations to pass through but there are a set of anions which may pass through CFTR. The conduction pathway of the channel can be split into two regions. The inner vestibule which is wide and covered with basic, positively charged amino acids and the selectivity filter which is formed by the narrowing of the protein in the upper part of the channel. h regions are hotspots for disease causing  mutations, iwth most mutations occuring on TM6 in TMD1 ().

F337 is the most important amino acid for selectivity, with the F337A mutation leading to an increase in conductivity and a loss in selectivity \cite{wei2016}. Bicarbonate (HCO$_3^-$ is known to have roughly 26\% the permeability of chloride through the channel. An interaction partner WNK1 has been shown to influence the selectivity of the channel \cite{kim2019}. The permeation of bicarbonate is very important physiologically because if a mutation permeates bicarbonate it means there is a high likelihood the patient will be pancreatic sufficient. This is being explored as a possible therapeutic strategy \cite{ferrera2021}. 

Compared to cation channels like Gramicidin and KcsA, CFTR is only weakly selective, permeating a large set of anions with varying radii and geometries \cite{}. Supposedly it is more permeant to lyotropic (low solvation energy anions) rather than cosmotropic anions (high solvation energy anions) indicating that partial dehydration of the anion is likely during conductance (CITATION NEEDED). The radius of dehydrated chloride ions is 1.7$\AA$ while a hydrated chloride ion has a radius of 3.2$\AA$, so even with this larger pore partial dehydration must take place \cite{yang2002}. 
