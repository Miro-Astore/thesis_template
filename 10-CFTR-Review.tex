...
All organs of the body are lined by cells called epithelia. They protect organs from trauma and infection. The most acute symptoms of CF are due to the dehydration of these epithelium in several. Of most concern is the epithelium in the lungs. When dehydrated, fine, motile structures called cilia, which line the epithelium collapse, rendering them unable to ``beat" and clear pathogens \cite{boucher2007}. Simultaneously, this dehydration causes the mucus which naturally lines the epithelium to thicken, as osmotic pressure leaches moisture away from it. 
...
From the perspective of a biophysicist, the cause of the disease is quite simple. The imbalance of salts and water across the epithelium is due to the misfunction of a single gene---an anion channel known as CFTR.
...
\begin{figure}
	...
	\end{center}
	\captionsetup{singlelinecheck = false, justification=raggedright}
	\caption[Western Blot Explanation] {\textbf{Western Blot Explanation}}{A) Output of a western blot experiment. WT-CFTR has much more abundant protein than delF508 \cite{chang2008}. B) The cell biology behind why delF508 is pathogenic. The protein misfolds as it is synthesised, so instead of being trafficked as normal, where it would be glycosylated in the Golgi, it is prematurely degraded by the proteosome \cite{lopes-pacheco2016a}.} 
\end{figure}
The above methods focus on measuring the electrical activity of the ion channels once they are at the cell membrane, they will only detect functional channels. In order to test the presence of ion channels within the epithilium, functional or otherwise we employ a technique called western blotting. In breif, this involves choosing an antibody which binds tightly to CFTR. The solution is washed to remove proteins not bound to this first antibody. A second antibody is then added to the solution which is engineereed to bind the first antibody. The resulting mixture is then injected into a gel which has a voltage applied to it, accelerating the proteins according to their molecular weight.\footnote{I really hope you're getting some appreciation for how hard biology is.} This separates out different proteins so different bands can be seen. When the gel is viewed, the darker the band, the more abundant the protein in the sample. See figure \ref{western_blot} to see hwo this can help us understand how measure the amount of protein at the cell surface. 

The combination of the above methods allows us to distinguish between the different classes of mutation which we outlined in section \ref{mutation_classes}.
