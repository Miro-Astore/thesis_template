...
NBD1 and 2 each have two binding sites for an ATP$+$MG^${2+}$ complex. This allows them to dimerise in what is known ``a head to tail" configuration where both NBDs make contact with both bound ATP molecules \cite{gout2012}  (Figure \ref{NBD_structure}). Usually, in an ABC transporter the NBDs would contain contain a ``Walker A motif" to help them bind ATP \textit {and} a ''Walker B" motif to catalyse the breakdown of ATP. A unique aspect of CFTR is that NBD1 is missing its Walker B motif, meaning one of the ATP binding sites is incapable of hydrolysing ATP. This ATP binding site is sometimes referred as ``site 1", ''the degenerate site" or ''the non-consensus site" and it plays an important role in regulating the gating ctivity of the channel \cite{yeh2022, csanady2019a}.
...
The above paragaphs outline the generally agreed upon details about the action of the NBDs and gating. There are two proposed models which debate how tightly correlated the opening of the channel is to the binding of ATP \cite{}. The first proposed models comprise that of rigid body motion of the NBDs and the TMDs this model suggest that ATP binding and NBD dimerisation is tightly coupled to channel opening. The second model suggests the presence of a conducting conformation where the NBDs are not tightly bound to one another \cite{}. It would seem that, although more coimplicated, the second model is both more consistent with recent findings in the literature andwould also explain some of the other controversies surrounding the structure and function of CFTR, such as the strange conformation of TM8 \cite{}.  
