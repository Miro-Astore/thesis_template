...
\begin{figure}
	\label{western_blot}
	\begin{center}
	\includegraphics[width=1\textwidth]{figures/western_blot_explanation.pdb}
	\end{center}
	\captionsetup{singlelinecheck = false, justification=raggedright}
	\caption[Western Blot Explanation] {\textbf{Western Blot Explanation}}{A) Output of a western blot experiment. WT-CFTR has much more abundant protein than delF508. B) The cell biology behind why delF508 is pathogenic. The protein misfolds as it is synthesised, so instead of being trafficked as normal, where it would be glycosylated in the Golgi, it is prematurely degraded by the proteosome.} 
\end{figure}
The above methods focus on measuring the electrical activity of the ion channels once they are at the cell membrane, they will only detect functional channels. In order to test the presence of ion channels within the epithilium, functional or otherwise we employ a technique called western blotting. In breif, this involves choosing an antibody which binds tightly to CFTR. A second antibody is then added to the solution which is engineereed to bind the first antibody. The resulting mixture is then injected into a gel which has a voltage applied to it, accelerating the proteins according to their molecular weight.\footnote{I really hope you're getting some appreciation for how hard biology is.} This separates out different proteins so different bands can be seen. THe darker the band, the more abbundant the protein in thes ample. See figure \ref{western_blot} to see hwo this can help us understand how measure the amount of protein at the cell surface. 
