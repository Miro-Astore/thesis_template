...
\chapter{Introduction: A Philosophy on Biological Physics}
\setcounter{page}{1}
\label{chap:intro}
...
This thesis seeks to apply a molecular philosophy on biophysics to demonstrate its ability to one day solve pressing problems in biology and medicine. In particular, we will use molecular dynamics (MD) to look at various modes of misfunction for an anion channel, the Cystic Frosts Transmembrane conductance regulator (CFTR).  These techniques allow us to formulate a model which we hope will direct research efforts and allow more patients suffering from Cystic Fibrosis (CF) access life saving medication.
...
Here, Schr\"oedinger has naturally chosen the formalism of interacting atoms, consistent with the statistical mechanics in equation \ref{hard_spheres}, to arrive at his model of an aperiodic crystal. In this thesis, as chapter \ref{chap:methods} justifies, we will use a more careful, but similar formalism to study the function of the CFTR protein. By chapter \ref{chap:conclusions} we will have sufficiently developed our model of CFTR to then use function of the CFTR gene as a means to describe disease. Using this model we will speculate where to direct the next efforts in the study of the molecular cause and treatment of CF. In this way we will have also built up the formalism of interacting atoms in order to develop a limited model for the function of an organism using the function of a single gene.
