...
The above model in figure \ref{drug_action_model} gives a rational, physical basis for the wide range of molecular phenotypes that CFTR modulators appear capable of treating. This same model would appear to argue that for most missense mutations we would expect them to respond to some sort of CFTR modulator. In short, the preceeding chapters have shown that CFTR may misfunction by a wide range of defects. 
\begin{itemize}
	\item Chapter \ref{chap:i37r} demonstrated that a novel form of pathogenesis arises from inter domain communications between the lasso motif and the R-domain.  This caused a class 2 gating defect. 
\item Chapter \ref{chap:r352q} showed how a conductance class defect may arise from the deletion of a postiive charge in the conduction pathway of the channel. The destruction of the salt bridge inside the channel may also affect the gating of the channel according to biochemical experiments \cite{}.
...
These molecular phenotypes are completely unique. At the atomic level, they share nothing in common. What is thus remarkable is that they all appear to respond to the same set of CFTR modulators in \textit{in vitro} experiments.
